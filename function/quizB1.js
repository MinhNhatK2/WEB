...
let timerB1; // Biến lưu trữ timer
let timeRemainingB1; // Biến lưu trữ thời gian còn lại
...
function submitQuiz() {
  ...
  sessionStorage.removeItem("timeRemainingB1"); // Xóa thời gian khi nộp bài
...
function startTimer() {
  ...
  const savedTime = sessionStorage.getItem("timeRemainingB1");
  timeRemainingB1 = savedTime ? parseInt(savedTime) : 20 * 60;

  timerB1 = setInterval(() => {
    if (timeRemainingB1 <= 0) {
      clearInterval(timerB1);
      submitQuiz();
      return;
    }

    const minutes = Math.floor(timeRemainingB1 / 60);
    const seconds = timeRemainingB1 % 60;
    countdownElement.textContent = `${minutes}:${
      seconds < 10 ? "0" + seconds : seconds
    }`;
    timeRemainingB1--;

    sessionStorage.setItem("timeRemainingB1", timeRemainingB1);
  }, 1000);
}
...
function evaluateTestResult() {
  ...
  return score >= 27 && !wrongType0 ? "Đạt" : "Trượt";
}
