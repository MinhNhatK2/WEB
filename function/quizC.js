...
let timerC; // Biến lưu trữ timer
let timeRemainingC; // Biến lưu trữ thời gian còn lại

// Lưu loại bài thi vào sessionStorage khi bắt đầu thi
sessionStorage.setItem("currentExam", "C");
...
function submitQuiz() {
  ...
  sessionStorage.removeItem("timeRemainingC"); // Xóa thời gian khi nộp bài
...
function startTimer() {
  ...
  const savedTime = sessionStorage.getItem("timeRemainingC");
  timeRemainingC = savedTime ? parseInt(savedTime) : 19 * 60;
  ...
  timerC = setInterval(() => {
    ...
    const minutes = Math.floor(timeRemainingC / 60);
    const seconds = timeRemainingC % 60;
    countdownElement.textContent = `${minutes}:${
      seconds < 10 ? "0" + seconds : seconds
    }`;
    timeRemainingC--;

    sessionStorage.setItem("timeRemainingC", timeRemainingC);
  }, 1000);
}
...
function evaluateTestResult() {
  ...
  return score >= 36 && !wrongType0 ? "Đạt" : "Trượt";
}
