...
let timerA1; // Biến lưu trữ timer
let timeRemainingA1; // Biến lưu trữ thời gian còn lại
...
function submitQuiz() {
  ...
  sessionStorage.removeItem("timeRemainingA1"); // Xóa thời gian khi nộp bài
...
function startTimer() {
  ...
  const savedTime = sessionStorage.getItem("timeRemainingA1");
  timeRemainingA1 = savedTime ? parseInt(savedTime) : 19 * 60;

  timerA1 = setInterval(() => {
    if (timeRemainingA1 <= 0) {
      clearInterval(timerA1);
      submitQuiz();
      return;
    }

    const minutes = Math.floor(timeRemainingA1 / 60);
    const seconds = timeRemainingA1 % 60;
    countdownElement.textContent = `${minutes}:${
      seconds < 10 ? "0" + seconds : seconds
    }`;
    timeRemainingA1--;

    sessionStorage.setItem("timeRemainingA1", timeRemainingA1);
  }, 1000);
}
...
function evaluateTestResult() {
  ...
  return score >= 21 && !wrongType0 ? "Đạt" : "Trượt";
}
